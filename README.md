# 🧠 Python & Data Science Personal Reference Notebook

> **My personal second brain for Python, Data Science, Machine Learning & AI.**

A structured, continuously evolving knowledge base for **fast recall, revision, interview preparation, and practical reference**.

This repository contains concise explanations, mental models, code snippets, syntax references, formulas, workflows, and important concepts that I learn throughout my journey from **Python → Data Science → Machine Learning → AI**.

---

## ⚠️ Work in Progress

This repository is **actively evolving**.

I am continuously adding, improving, and reorganizing notes as I learn new concepts and gain a deeper understanding of them.

> The goal is not to collect everything. The goal is to build a **high-signal technical reference that I can actually use and recall**.

---

# ⚡ Quick Navigation Index

## 📁 1. Core & Advanced Python

- [Core Syntax & Data Structures](./01_core_python/) — Lists, tuples, dictionaries, sets, slicing, comprehensions, functions.
- [Advanced Python](./02_advanced_python/README.md) — File handling, exceptions, recursion, namespaces, decorators, generators, and GUI development with Streamlit.

---

## 📁 2. Object-Oriented Programming

- [OOP Principles: Basic to Advanced](./03_oop_principles/README.md) — Eight organized chapters covering classes, properties, inheritance, protocols, dataclasses, descriptors, metaclasses, and a tested mini-project.
- [Design Patterns](./03_oop_principles/patterns.md) — SOLID, strategy, factories, adapters, composition, repositories, and design tradeoffs.

---

## 📁 3. Data Science

- [Data Science Library Index](./04_data_science/README.md) — Seven separate guides with a reading order, examples, and practice tasks.
- [NumPy](./04_data_science/numpy.md), [pandas](./04_data_science/pandas.md), [SciPy](./04_data_science/scipy.md), and [Polars](./04_data_science/polars.md) — Numerical computing, data analysis, scientific algorithms, and lazy table processing.
- [Matplotlib](./04_data_science/matplotlib.md), [Seaborn](./04_data_science/seaborn.md), and [Plotly](./04_data_science/plotly.md) — Scientific, statistical, and interactive visualization.

---

## 📁 4. Machine Learning 🤖

- [ML Fundamentals](./05_machine_learning/) — Core ML concepts, learning paradigms, ML development lifecycle, common challenges, and fundamental mental models.
- [Math & Intuition](./05_machine_learning/math.md) — Mathematical intuition behind important ML algorithms and concepts.
- [Feature Engineering](./05_machine_learning/engineering.md) — Scaling, encoding, missing values, feature creation, transformations, and pipelines.
- [Evaluation Metrics](./05_machine_learning/metrics.md) — Accuracy, precision, recall, F1-score, ROC-AUC, PR-AUC, MAE, MSE, RMSE, R², and related concepts.

---

## 📁 5. Artificial Intelligence & Deep Learning 🚀

- [Deep Learning Fundamentals](./06_artificial_intelligence/) — Neural networks, activation functions, forward propagation, backpropagation, weights, biases, and optimization.
- [Frameworks](./06_artificial_intelligence/frameworks.md) — PyTorch/TensorFlow references, model building, custom layers, datasets, and training loops.
- [GenAI & LLMs](./06_artificial_intelligence/genai.md) — Prompt engineering, LLM fundamentals, fine-tuning, RAG, embeddings, APIs, and AI application architectures.

---

## 📁 6. Version Control & Open Source

- [Git, GitHub, GitLab, and Open Source for ML](./version-control.md) — Commands and use cases, branching, conflicts, recovery, PR/MR workflows, ML reproducibility, Git LFS, CI, and GSoC preparation.

---

## 📁 7. SQL & Relational Databases

- [SQL Learning Index](./08_sql/README.md) — A structured beginner-to-advanced course with a shared practice database, joins, subqueries, CTEs, window functions, optimization, transactions, analytics, and worked exercises.

---

## 📁 8. Excel & Spreadsheet Analysis

- [Excel Learning Index](./09_excel/README.md) — Structured beginner-to-advanced notes covering formulas, lookups, PivotTables, dashboards, Power Query, DAX, statistics, VBA, Office Scripts, Python integration, and worked practice projects.

---

## 🚀 How to Use This Repo

1. Use **`Ctrl + F`** (or `Cmd + F`) on this page to search for a specific keyword.
2. Click the direct link to jump into the dedicated notebook/markdown folder.
3. Copy, paste, and adapt the minimal code snippets as needed.

---

# 🎯 Long-Term Goal

Build a strong foundation in:

- Python engineering
- Data Science
- Statistics & Mathematics
- Machine Learning
- Deep Learning
- Artificial Intelligence
- Generative AI
- Software Engineering
- ML/AI system design
- Deployment and MLOps

The repository is a record of that learning process.

---

## ⭐ Core Principle

> **Learn → Understand → Build → Explain → Record → Recall → Improve**
